...
function displayAnalytics(analytics, genTime) {
    ...
    document.getElementById('metricsGrid').innerHTML = `
        ...
        <div class="stat-card" onclick="toggleShopsLeftDetails()"><div class="stat-icon">🏪</div><div class="stat-content"><div class="stat-label">Pending</div><div class="stat-value">${metrics.totalShopsLeft}</div></div></div>
    `;
    ...
    if (analytics.insights) {
        renderInsightsList('insightsList', analytics.insights);
        document.getElementById('insightsSection').style.display = 'block';
    }
}

function renderInsightsList(id, insights) {
...
function renderPerformerList(id, list, isTop) {
    ...
    // 3. Take only top 5 groups and render
...
async function exportSectorCard(btn, secName, type) {
    ...
    try {
        ...
        if (type === 'image') {
            ...
            link.download = `Balance_${secName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().getTime()}.png`;
            link.href = imgData;
            link.click();
        } else if (type === 'pdf') {
            ...
            pdf.save(`Balance_${secName.replace(/[^a-zA-Z0-9]/g, '_')}_${new Date().getTime()}.pdf`);
        }
    } catch (e) {
...
function displayWELFAREAnalytics(analytics, v, genTime) {
    ...
    // AI Insights
    if (analytics.insights) {
        renderInsightsList('welfareInsightsList', analytics.insights);
...
function displayNfsaDaterangeAnalytics(analytics) {
    ...
    if (analytics.insights) {
        renderInsightsList('drInsightsList', analytics.insights);
