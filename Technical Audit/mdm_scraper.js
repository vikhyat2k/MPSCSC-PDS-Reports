...
class MDMScraper {
    ...
    async _extractDepotShops(depot, depotIndex) {
        ...
        // Wait for #fpsreport to load (getreportfps posts to MDM_allotment_fps.jsp
        // and injects result into #detailsEDfps, which contains table#fpsreport)
        // Use 45s timeout for large depots like BHIMPUR (104 shops)
        ...
        // Retry once if fpsreport has too few rows (server still loading)
        ...
        if (rowCount <= 5) {
            ...
            // Re-click the depot using eval again
        ...
        const shops = await this.page.evaluate((depotNameStr) => {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                // Log first row's all cell values to verify column indices
                if (result.length === 0) {
                    sampleRow = Array.from(cells).map((c, idx) => `[${idx}]=${c.innerText.trim().substring(0, 20)} `);
                }

                // Column mapping for #fpsreport (MDM_allotment_fps.jsp) — 18 columns:
                // [0]=SL, [1]=ShopCode, [2]=Schools, [3]=Inmates
                // [4]=WhtRequired, [5]=WhtCB, [6]=WhtAllotted, [7]=WhtRO, [8]=WhtDispatched, [9]=WhtReceivedFPS, [10]=WhtIssued
                // [11]=RiceRequired, [12]=RiceCB, [13]=RiceAllotted, [14]=RiceRO, [15]=RiceDispatched, [16]=RiceReceivedFPS, [17]=RiceIssued
                result.push({
                    shopCode,
        ...
        if (!Array.isArray(shops) && shops.tableFound) {
            console.log(`   #fpsreport found: ${shops.totalRows} rows, ${shops.firstDataCols} data columns`);
            if (shops.sampleRow) {
                console.log(`   Sample first row cells: ${shops.sampleRow.join(' | ')} `);
            }
