...
class WelfareScraper {
    ...
    async _extractDepotShops(depot, depotIndex) {
        ...
        await this.page.evaluate(() => {
            const container = document.getElementById('detailsWelInst');
            if (container) container.innerHTML = '';
        });

        // Trigger depot report load by evaluating the onclick directly
        const ajaxResult = await this.page.evaluate((onclickStr) => {
            // Ensure containers exist for the native AJAX to populate into
            if (!document.getElementById('detailsWelInst')) {
                const c1 = document.createElement('div');
                c1.id = 'detailsWelInst';
                document.body.appendChild(c1);
            }
            if (!document.getElementById('detailsWelInstFPS')) {
                const c2 = document.createElement('div');
                c2.id = 'detailsWelInstFPS';
        ...
        if (!ajaxResult.success) {
            console.warn(`⚠️ Could not load depot SL ${depot.slNo}: ${ajaxResult.msg}`);
            return [];
        }
        console.log(`   Depot load initiated via ${ajaxResult.method} for ${ajaxResult.depotCode || depot.name}`);

        await this._waitForLoading();

        // Wait for #instreportdd
        const waitForFpsReport = async (timeoutMs = 45000) => {
            await this.page.waitForFunction(() => {
                ...
                console.warn(`⚠️ instreportdd table timeout for ${depot.depotName || depot.name}`)
            );
        };

        await waitForFpsReport(90000);
        ...
        // Parse institute data from #instreportdd
        // Welfare scheme table — 2 commodities: Wheat + Fortified Rice
        // Expected column layout (similar to ICDS 18-col without FSalt):
        // [0]=SL, [1]=InstCode, [2]=Institutes, [3]=Beneficiaries
        // Wheat:  [4]=Required, [5]=CB, [6]=Allotted, [7]=RO, [8]=Dispatch, [9]=ReceivedFPS, [10]=Issued
        // Rice:   [11]=Required, [12]=CB, [13]=Allotted, [14]=RO, [15]=Dispatch, [16]=ReceivedFPS, [17]=Issued
        const shops = await this.page.evaluate((depotNameStr) => {
            ...
            for (let i = 0; i < rows.length; i++) {
                ...
                // Welfare institute codes may be numeric
                ...
                // Adaptive column mapping based on actual column count
                // Welfare 23-col layout:
                // [0-4]: Metadata
                // Rice [5-13]: [9]=TotalAllot, [11]=Dispatch, [12]=Received
                // Wheat [14-22]: [18]=TotalAllot, [20]=Dispatch, [21]=Received
                ...
                if (colCount >= 23) {
                    ...
                    // Fallback for unexpected 18-col (if any)
                    wA = 6; wDi = 8; wRe = 9;
                    rA = 13; rDi = 15; rRe = 16;
                } else {
                    // Minimal — skip
        ...
        if (!Array.isArray(shops) && shops.tableFound) {
            console.log(`   #fpsreport: ${shops.totalRows} rows, ${shops.firstDataCols} cols`);
            if (shops.sampleRow) console.log(`   Sample: ${shops.sampleRow.slice(0, 10).join(' | ')}`);
