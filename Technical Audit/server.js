                ...
                try {
                    ...
                    for (const roType of roTypes) {
                        ...
                        try {
                            ...
                            // CRITICAL: Abort only if 'Regular' fails — it contains ~97% of the data.
                            // Portability/Extra failures are non-critical; log a warning and proceed.
                            // The report will still be generated with Regular data if Portability fails.
                            if (roType === 'Regular') {
                                throw new Error(`CRITICAL: Regular RO type extraction failed — ${err.message}. Cannot generate accurate report.`);
                            }
...
function computeNFSADaterangeAnalytics(processedResult, fromDate, toDate, allotmentMapping = null, shopAllotmentMapping = null) {
    ...
    const groupPerformersHelper = (data, sortOrder = 'desc', limit = 5) => {
        ...
        data.forEach(s => {
            ...
            groups[t].dispatchQty += parseFloat(s.dispatch || 0);
    ...
    const bottomPerformers = groupPerformersHelper(basePool, 'asc', 5);
    ...
    try {
        ...
        (async () => {
            ...
            try {
                ...
                const reportFile = await nfsaDaterangeExcelGenerator.generateReport(processedResult, fromDate, toDate);
...
app.post('/api/generate-pdf/:id', async (req, res) => {
    try {
        ...
        if (scheme === 'mdm') {
            ...
            pdfFile = await nfsaDaterangePdfGenerator.generateReport(processedData, fromD, toD);
        } else {
            // Default to NFSA
...
function computeICDSAnalytics(processedResult) {
    ...
    const matrix = activeSectors.map(s => ({
        ...
        totalReceiptPct: s.totalAllotted > 0
            ? cap(parseFloat(((s.totalReceived / s.totalAllotted) * 100).toFixed(2))) : 0
    }));
        ...
        for (const selection of selectedSchemes) {
            ...
            if (!report) {
                ...
                try {
                    if (scheme === 'nfsa') {
                        ...
                        for (const roType of roTypes) {
                            try {
                                ...
                                if (roType === 'Regular') {
                                    throw new Error(`CRITICAL: Regular RO extraction failed - ${err.message}`);
                                }
                            }
                        }
                    ...
                    console.error(`❌ Auto-generation failed for ${scheme} ${month}/${year}:`, genErr);
                    generationErrors.push(`[${scheme.toUpperCase()} ${month}/${year}] ${genErr.message}`);
                    // Fallback: try the most recent cached report from DB
                    if (forceRefresh) {
                    ...
                    if (!fs.existsSync(finalPdfPath) || forceRefresh) {
                        try {
                            ...
                            if (scheme === 'nfsa_daterange') {
                                ...
                                pdfResult = await new NFSADaterangePdfGenerator().generateReport(procData, actualFromDate, actualToDate);
                            } else if (scheme === 'mdm') {
