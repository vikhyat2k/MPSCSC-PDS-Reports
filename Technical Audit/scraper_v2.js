...
class SCMScraper {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        await this.page.evaluate(() => {
            const img = document.querySelector('#captcha_img, img[src*="captcha"]');
            if (img) {
                img.style.filter = 'contrast(200%) grayscale(100%) brightness(150%)';
        ...
        const captchaElement = await this.page.$('#captcha_img');
        let captchaBuffer;

        if (captchaElement) {
          captchaBuffer = await captchaElement.screenshot();
        } else {
          const captchaImg = await this.page.$('img[src*="captcha"]');
          if (captchaImg) {
            captchaBuffer = await captchaImg.screenshot();
          } else {
        ...
        if (process.env.TWOCAPTCHA_API_KEY) {
          ...
          cleanedCaptcha = await this.solveWith2Captcha(base64Image) || '';
        }
        ...
        if (!cleanedCaptcha) {
          ...
          const tempImagePath = path.join(process.cwd(), 'logs', 'temp_captcha_audit.png');
          fs.writeFileSync(tempImagePath, captchaBuffer);
          const { data } = await this.ocrWorker.recognize(tempImagePath);
          cleanedCaptcha = data.text.replace(/[^a-zA-Z0-9]/g, '').trim();
          console.log(`   OCR Result Conf: ${data.confidence}`);
        }

        console.log(`   OCR Result: "${cleanedCaptcha}"`);

        if (!cleanedCaptcha || cleanedCaptcha.length < 3) {
          console.log('⚠️ OCR result invalid, reloading page for fresh CAPTCHA...');
          try {
            await this.page.goto(this.baseURL, { waitUntil: 'domcontentloaded', timeout: 60000 });
            const userInput = await this.page.$('input[name="userName"], input[name="username"], input[type="text"]').catch(() => null);
            const passInput = await this.page.$('input[name="password"], input[type="password"]').catch(() => null);
            if (userInput) { await userInput.click({clickCount:3}); await userInput.type(process.env.SCM_USERNAME || '', {delay:80}); }
            if (passInput) { await passInput.click({clickCount:3}); await passInput.type(process.env.SCM_PASSWORD || '', {delay:80}); }
          } catch (reloadErr) {
        ...
        // Enter CAPTCHA
        const captchaInput = await this.page.$('input[name="captcha"], #captcha, input[placeholder*="Captcha"]');
        if (captchaInput) {
          await captchaInput.click({ clickCount: 3 }); // Select all
          await captchaInput.type(cleanedCaptcha, { delay: 100 });

          // Click Submit
          const submitBtn = await this.page.$('input[type="submit"], button[type="submit"], input[value="Login"]');
          if (submitBtn) {
            // Click and wait for navigation OR error
            const navPromise = this.page.waitForNavigation({ waitUntil: 'networkidle2', timeout: 60000 }).catch(() => null);
            ...
            console.log('❌ Login attempt failed. Refreshing CAPTCHA for retry...');
            // Try to detect specific error but refresh anyway
            const errorText = await this.page.evaluate(() => document.body.innerText);
            if (errorText.includes('Invalid') || errorText.includes('Captcha')) {
              console.log('   (Detected Invalid CAPTCHA error message)');
            }

            // Clear input for next attempt
            await captchaInput.click({ clickCount: 3 }).catch(() => null);
            await this.page.keyboard.press('Backspace').catch(() => null);
            
            // Refresh CAPTCHA image for next attempt
            const refreshBtn = await this.page.$('img[src*="refresh"]');
            if (refreshBtn) {
        ...
        // If navigation occurred during attempt, we might already be logged in
        if (await this.verifyLogin().catch(()=>false)) return true;
      }
      
      // Delay between attempts to wait for new captcha to generate
  ...
  async login(username, password, maxRetries = 5, onProgress = null) {
    ...
    try {
      ...
      try {
        ...
        console.warn(`⚠️ Navigation warning (networkidle2): ${err.message}. Retrying with 'load'...`);
        await this.page.goto(this.baseURL, {
          waitUntil: 'load',
          timeout: 60000
        });
  ...
  async selectFormAndGetDetails(month, year, roType) {
    ...
    try {
      ...
      // Select Month (1st select - index 0)
      const monthSelect = selectInfo[0];
      const monthSelector = monthSelect.id ? `#${monthSelect.id}` : (monthSelect.name ? `select[name="${monthSelect.name}"]` : 'select:nth-of-type(1)');
      ...
      // Select Year (2nd select - index 1)
      const yearSelect = selectInfo[1];
      const yearSelector = yearSelect.id ? `#${yearSelect.id}` : (yearSelect.name ? `select[name="${yearSelect.name}"]` : 'select:nth-of-type(2)');
      ...
      // Select RO Type (3rd select - index 2)
      const roSelect = selectInfo[2];
      const roSelector = roSelect.id ? `#${roSelect.id}` : (roSelect.name ? `select[name="${roSelect.name}"]` : 'select:nth-of-type(3)');
    ...
    try {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        if (await this.checkSessionError()) {
          ...
          await this.login(process.env.SCM_USERNAME, process.env.SCM_PASSWORD);
          await this.page.goto(`${process.env.SCM_URL}/RO_ABSTRACT`, { waitUntil: 'networkidle2' });
          // Re-select form
    ...
    try {
      ...
      for (let i = 0; i < summary.rows.length; i++) {
        ...
        while (retryAttempt < 3 && !fetchedValidDispatch) {
          ...
          // Validate SCM Backend Timeout (Abstract promised Dispatch, Details gave 0)
          const extractedDispatch = details.reduce((sum, s) => sum + (s.dispatch || 0), 0);

          if (expectedDispatch >= 50 && extractedDispatch < 1) {
            console.error(`❌ SCM Backend Timeout Detected for ${name}! Expected ~${expectedDispatch.toFixed(2)} Qt Dispatch, got 0 Qt. HTML returned empty columns.`);
            retryAttempt++;
            ...
            if (retryAttempt < 3) {
              ...
              if (roType === 'Regular') {
                throw new Error(`STRICT_INTEGRITY_FAILURE: Could not extract details for Regular sector ${name} after 3 attempts.`);
              }
              fetchedValidDispatch = true; // Give up and move on for non-Regular
            }
          } else {
