...
class WelfareDataProcessor {
    ...
    processData(rawData, summaryTotals) {
        ...
        shops.forEach(shop => {
            ...
            const riceAlloc = parseKg(shop.riceAllotted || shop.fortifiedRiceAllotted || 0);
            const totalAlloc = wheatAlloc + riceAlloc;

            const wheatDisp = parseKg(shop.wheatDispatched || 0);
            const riceDisp = parseKg(shop.riceDispatched || shop.fortifiedRiceDispatched || 0);
            const totalDisp = wheatDisp + riceDisp;

            const wheatRec = parseKg(shop.wheatReceived || 0);
            const riceRec = parseKg(shop.riceReceived || shop.fortifiedRiceReceived || 0);
            ...
            sectorsMap[sectorId].shops.push({
                ...
                beneficiaries: shop.beneficiaries || 0,
