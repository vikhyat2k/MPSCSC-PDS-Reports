...
class ExcelGenerator {
    ...
    async generateReport(processedResult, month, year) {
        ...
        worksheet.addRow([`मासिक आवंटन एवं परिवहन रिपोर्ट (NFSA) - ${monthName} ${year}`]);
        worksheet.addRow([]);
        ...
        if (processedResult && processedResult.sectors) {
            ...
            worksheet.addRow([
                '', 'कुल योग', '',
                (totals.totalAllocation || 0).toFixed(2),
                (totals.totalDispatch || 0).toFixed(2),
                (totals.totalPOSReceipt || 0).toFixed(2),
        ...
        const filename = `NFSA_Report_${month}_${year}_${uuidv4().substring(0,8)}.xlsx`;
