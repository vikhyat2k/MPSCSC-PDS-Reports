...
class PDFGenerator {
    ...
    async generateReport(processedData, month, year) {
        ...
        <body>
            ...
            <h2>मासिक आवंटन एवं परिवहन रिपोर्ट (NFSA) - ${monthName} ${year}</h2>
            <table>
                <thead>
                    <tr>
                        <th>स. क्र.</th>
        ...
        if (processedData && processedData.sectors) {
            processedData.sectors.forEach((sector, i) => {
                htmlContent += `
                    <tr>
                        ...
                        <td>${(sector.posReceipt || 0).toFixed(2)}</td>
            ...
            htmlContent += `
                <tr style="font-weight: bold;">
                    ...
                    <td>${(totals.totalPOSReceipt || 0).toFixed(2)}</td>
        ...
        const filename = `NFSA_Report_${month}_${year}_${uuidv4().substring(0,8)}.pdf`;
