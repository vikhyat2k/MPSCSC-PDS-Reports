...
class MDMDataProcessor {
    ...
    processData(rawData, summaryTotals) {
        ...
        shops.forEach(shop => {
            ...
            // MDM typical commodities: wheat, fortifiedRice
            // The scraper returns actual quintals, but the user spec asks to divide by 100 "consistent with balancesReportGenerator.js"
            // Wait, the scrapers already returned 1.64 for 1.64 quintals?
            // Actually, the prompt says "dividing raw portal values by 100 (Kg -> Quintal) consistent with balancesReportGenerator.js's parseKg pattern."
            // I will use parseKg as specified.
            const wheatAlloc = parseKg(shop.wheatAllotted || 0);
            const riceAlloc = parseKg(shop.fortifiedRiceAllotted || shop.riceAllotted || 0);
            const totalAlloc = wheatAlloc + riceAlloc;

            const wheatDisp = parseKg(shop.wheatDispatched || 0);
            const riceDisp = parseKg(shop.fortifiedRiceDispatched || shop.riceDispatched || 0);
            const totalDisp = wheatDisp + riceDisp;

            const wheatRec = parseKg(shop.wheatReceived || 0);
            const riceRec = parseKg(shop.fortifiedRiceReceived || shop.riceReceived || 0);
        ...
        // if summaryTotals is provided, we might want to use it for verification.
        // The user specified MDM uses: totalAllotted, totalDispatched, totalReceived, totalDispatchPct
